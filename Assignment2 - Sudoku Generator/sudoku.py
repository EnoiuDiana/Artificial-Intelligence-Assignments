...
class SudokuUI(Frame):
    ...
    def __initUI(self):
        self.game_window.geometry('520x525')
    ...
    def __config_game_buttons(self):
        ...
        self.submit_btn = Button(self.buttons_frame, text='Submit solution',
                                 font=('arial', 11),
                                 width=30, height=2, bg='#e6fff2', cursor='hand2',
...
class SudokuGame(object):
    ...
    def get_a_new_board(self):
        sudokuBoardGenerator = generator.GenerateSudokuBoard()
        board_with_numbers = sudokuBoardGenerator.generate_board_with_mace4()
        self.board = self.__transform_int_to_str(board_with_numbers)
        self.saved_copy_board = self.__save_a_copy_board()
        self.solved_board = self.__save_a_copy_board()

    def __transform_int_to_str(self, board_with_int):
        board_with_str = []
        for r in range(9):
            new_list = []
            for c in range(9):
                new_list.append(str(board_with_int[r][c]))
            board_with_str.append(new_list)
        return board_with_str
