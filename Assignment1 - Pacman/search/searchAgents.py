        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                for corner in cornersVisited:
                    successorCorners.append(corner)
                if nextNode in self.corners:
                    if nextNode not in successorCorners:
                        successorCorners.append(nextNode)
                cost = 1
                successors.append(((nextNode, successorCorners), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisitedCorners) != 0:
        closestCorner = closestPoint(referencePoint, unvisitedCorners)
        heuristic += util.manhattanDistance(referencePoint, closestCorner)
...
def closestPoint(fromPoint, candidatesList):
    ...
    foundClosestPoint = candidatesList[0]
    closestCost = util.manhattanDistance(fromPoint, foundClosestPoint)
...
def foodHeuristic(state, problem):
    ...
    heuristic = mazeDistance(closestFood, position, gameState)
    return heuristic
